...
async def paid_next_page(bot, query):
    ...
    if settings['button']:
        ...
        search_results_text = []
        for file in files:
            shortlink = f"https://telegram.me/{temp.U_NAME}?start=User-{query.from_user.id}_{file.file_id}"
...
async def paid_filter(client, msg, spoll=False):
    ...
    search_results_text = []
    for file in files:
        shortlink = f"https://telegram.me/{temp.U_NAME}?start=User-{message.from_user.id}_{file.file_id}"
        file_link = f"🎬 [{get_size(file.file_size)} | {await replace_blacklist(file.file_name, blacklist)}]({shortlink})"
        search_results_text.append(file_link)
