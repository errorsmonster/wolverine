    ...
    if group_id in AUTH_GROUPS:
        ...
        if await db.get_chat(group_id):
            ...
            if api:
                ...
                await message.reply_text(text=f"This group isn't configured, To configure, please send /configure", reply_markup=button, disable_web_page_preview=True)
        else:
        ...
        if 2 < len(message.text) < 100:
            ...
            if not files:
                if settings["spell_check"]:
                    ...
                    return await message.reply_text("No Results Found For - {search}. Try Another Keyword.")
        else:
            return
    else:
